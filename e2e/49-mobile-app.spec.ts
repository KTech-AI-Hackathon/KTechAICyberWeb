...
test.describe('Mobile App Service Page', () => {
  ...
  test('should display benefits section', async ({ page }) => {
    ...
    // Check benefits have checkmarks
  ...
  test('CTA button should navigate to contact section', async ({ page }) => {
    ...
    // Should navigate to home with contact hash
  ...
  test('should have proper heading hierarchy for accessibility', async ({ page }) => {
    ...
    await expect(h2s).toHaveCount(5) // hero-title, 4 section titles
  })
  ...
  test('should support language switching', async ({ page }) => {
    ...
    await expect(page.locator('.hero-title')).toContainText('Mobile')
    ...
    if (await langSwitcher.isVisible()) {
      ...
      await expect(page.locator('.hero-title')).toContainText('移动')
    }
  })
...
test.describe('Mobile App Service Page - Navigation', () => {
  test('should be accessible from direct URL', async ({ page }) => {
    await page.goto('/services/mobile-app')
    await expect(page.locator('.mobile-app-page')).toBeVisible()
  })

  test('related services links should be functional', async ({ page }) => {
    await page.goto('/services/mobile-app')
