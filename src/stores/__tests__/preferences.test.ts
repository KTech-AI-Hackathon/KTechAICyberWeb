...
describe('usePreferencesStore (theme toggle)', () => {
  ...
  describe('rumEnabled (#187 RUM beacon opt-in)', () => {
    it('defaults to false (inert until explicitly enabled)', () => {
      const store = usePreferencesStore()
      expect(store.rumEnabled).toBe(false)
    })

    it('setRumEnabled(true) flips + persists the choice', () => {
      const store = usePreferencesStore()
      store.setRumEnabled(true)
      expect(store.rumEnabled).toBe(true)
      const raw = JSON.parse(localStorage.getItem(PREFERENCES_STORAGE_KEY) || '{}')
      expect(raw.rumEnabled).toBe(true)
    })

    it('setRumEnabled(false) flips back off + persists', () => {
      const store = usePreferencesStore()
      store.setRumEnabled(true)
      store.setRumEnabled(false)
      expect(store.rumEnabled).toBe(false)
      const raw = JSON.parse(localStorage.getItem(PREFERENCES_STORAGE_KEY) || '{}')
      expect(raw.rumEnabled).toBe(false)
    })

    it('coerces non-boolean truthy to true only for explicit true', () => {
      const store = usePreferencesStore()
      // Only strict true enables; a truthy string must NOT flip it on.
      store.setRumEnabled('yes' as any)
      expect(store.rumEnabled).toBe(false)
      store.setRumEnabled(true)
      expect(store.rumEnabled).toBe(true)
    })
    ...
    it('hydrates a persisted true value', () => {
      localStorage.setItem(
        PREFERENCES_STORAGE_KEY,
        JSON.stringify({ theme: 'dark', language: 'en', rumEnabled: true }),
      )
      const store = usePreferencesStore()
      expect(store.rumEnabled).toBe(true)
    })

    it('reset() clears rumEnabled back to false', () => {
      const store = usePreferencesStore()
      store.setRumEnabled(true)
      store.reset()
      expect(store.rumEnabled).toBe(false)
    ...
    it('persist() keeps theme + language alongside rumEnabled (no regression)', () => {
      const store = usePreferencesStore()
      store.setTheme('dark')
      store.setLanguage('zh')
      store.setRumEnabled(true)
      const raw = JSON.parse(localStorage.getItem(PREFERENCES_STORAGE_KEY) || '{}')
      expect(raw).toEqual({ theme: 'dark', language: 'zh', rumEnabled: true })
    })
  })
})
