<template>
  <main
    id="main-content"
    class="cross-border-payment"
    :aria-label="t('a11y.mainLabel')"
    role="main"
  >
    <!-- Animated background grid -->
    <div class="grid-bg"></div>
    <div class="grid-bg grid-bg-2"></div>

    <!-- Loading skeleton -->
    <div v-if="loading" class="cbp__skeleton" role="status" :aria-label="t('a11y.loadingLabel')">
      <div class="cbp__skeleton-block cbp__skeleton-block--lg"></div>
      <div class="cbp__skeleton-block cbp__skeleton-block--md"></div>
      <div class="cbp__skeleton-block cbp__skeleton-block--sm"></div>
      <div class="cbp__skeleton-block cbp__skeleton-block--md"></div>
    </div>

    <!-- Real content -->
    <div v-else class="cbp__content">
      <!-- Breadcrumb Navigation -->
      <nav class="cbp__breadcrumb" aria-label="Breadcrumb">
        <router-link to="/" class="cbp__breadcrumb-link">
          {{ t('nav.home') }}
        </router-link>
        <span class="cbp__breadcrumb-separator" aria-hidden="true">/</span>
        <span class="cbp__breadcrumb-current">
          {{ t('services.crossBorderPayment.title') }}
        </span>
      </nav>

      <!-- Hero Section -->
      <section class="cbp__hero" aria-labelledby="cbp-hero-heading">
        <div class="cbp__hero-content">
          <div class="cbp__hero-icon-wrapper">
            <div class="cbp__hero-icon" aria-hidden="true">🌐</div>
          </div>
          <h1 id="cbp-hero-heading" class="cbp__hero-title neon-text">
            {{ t('services.crossBorderPayment.hero.heading') }}
          </h1>
          <p class="cbp__hero-subtitle">
            {{ t('services.crossBorderPayment.hero.description') }}
          </p>
          <div class="cbp__hero-cta">
            <router-link
              to="/contact"
              class="cbp__hero-button neon-border"
              :aria-label="t('services.crossBorderPayment.cta.ariaLabel')"
            >
              {{ t('services.crossBorderPayment.cta.button') }}
            </router-link>
          </div>
        </div>
      </section>

      <!-- Overview Section -->
      <section class="cbp__overview" aria-labelledby="cbp-overview-heading">
        <h2 id="cbp-overview-heading" class="cbp__section-title">
          {{ t('services.crossBorderPayment.overview.heading') }}
        </h2>
        <p class="cbp__overview-text">
          {{ t('services.crossBorderPayment.overview.description') }}
        </p>
      </section>

      <!-- Capabilities Section -->
      <section class="cbp__capabilities" aria-labelledby="cbp-capabilities-heading">
        <h2 id="cbp-capabilities-heading" class="cbp__section-title">
          {{ t('services.crossBorderPayment.capabilities.heading') }}
        </h2>
        <div class="cbp__cards-grid">
          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">🏦</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.capabilities.c1.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.capabilities.c1.description') }}
            </p>
          </article>

          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">⚡</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.capabilities.c2.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.capabilities.c2.description') }}
            </p>
          </article>

          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">🛡️</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.capabilities.c3.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.capabilities.c3.description') }}
            </p>
          </article>
        </div>
      </section>

      <!-- Core Features Section -->
      <section class="cbp__core-features" aria-labelledby="cbp-core-features-heading">
        <h2 id="cbp-core-features-heading" class="cbp__section-title">
          {{ t('services.crossBorderPayment.coreFeatures.heading') }}
        </h2>
        <div class="cbp__cards-grid">
          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">🏦</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.coreFeatures.corridors.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.coreFeatures.corridors.description') }}
            </p>
          </article>

          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">⚡</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.coreFeatures.settlement.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.coreFeatures.settlement.description') }}
            </p>
          </article>

          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">💱</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.coreFeatures.fxOrchestration.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.coreFeatures.fxOrchestration.description') }}
            </p>
          </article>

          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">📋</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.coreFeatures.valueAdded.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.coreFeatures.valueAdded.description') }}
            </p>
          </article>
        </div>
      </section>

      <!-- Tech Features Section -->
      <section class="cbp__tech-features" aria-labelledby="cbp-tech-features-heading">
        <h2 id="cbp-tech-features-heading" class="cbp__section-title">
          {{ t('services.crossBorderPayment.techFeatures.heading') }}
        </h2>
        <div class="cbp__cards-grid">
          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">🔌</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.techFeatures.apiLayer.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.techFeatures.apiLayer.description') }}
            </p>
          </article>

          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">🛡️</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.techFeatures.complianceEngine.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.techFeatures.complianceEngine.description') }}
            </p>
          </article>

          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">🔄</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.techFeatures.reconciliation.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.techFeatures.reconciliation.description') }}
            </p>
          </article>

          <article class="cbp__card">
            <div class="cbp__card-icon neon-border" aria-hidden="true">🏗️</div>
            <h3 class="cbp__card-title">
              {{ t('services.crossBorderPayment.techFeatures.integration.title') }}
            </h3>
            <p class="cbp__card-description">
              {{ t('services.crossBorderPayment.techFeatures.integration.description') }}
            </p>
          </article>
        </div>
      </section>

      <!-- CTA Section -->
      <section class="cbp__cta" aria-labelledby="cbp-cta-heading">
        <h2 id="cbp-cta-heading" class="cbp__cta-title neon-text">
          {{ t('services.crossBorderPayment.cta.heading') }}
        </h2>
        <p class="cbp__cta-description">
          {{ t('services.crossBorderPayment.cta.description') }}
        </p>
        <router-link
          to="/contact"
          class="cbp__cta-button neon-border"
          :aria-label="t('services.crossBorderPayment.cta.ariaLabel')"
        >
          {{ t('services.crossBorderPayment.cta.button') }}
        </router-link>
      </section>

      <!-- Back to Services -->
      <div class="cbp__back">
        <router-link to="/" class="cbp__back-link">
          <span class="cbp__back-icon" aria-hidden="true">←</span>
          {{ t('services.backToServices') }}
        </router-link>
      </div>
    </div>
  </main>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import { useLanguage } from '../composables/useLanguage'

const { t } = useLanguage()

// Loading skeleton state: show a skeleton until first paint settles.
const loading = ref(true)

onMounted(() => {
  window.scrollTo({ top: 0, behavior: 'smooth' })

  // document <title> is owned by App.vue's useHead (#260).
  const metaName = 'description'
  let descriptionMeta = document.querySelector(`meta[name="${metaName}"]`)
  if (!descriptionMeta) {
    descriptionMeta = document.createElement('meta')
    descriptionMeta.setAttribute('name', metaName)
    document.head.appendChild(descriptionMeta)
  }
  descriptionMeta.setAttribute('content', t('services.crossBorderPayment.subtitle'))

  const cards = document.querySelectorAll('.cbp__card')
  for (let i = 0; i < cards.length; i++) {
    cards[i].style.animationDelay = `${i * 0.1}s`
  }

  loading.value = false
})
</script>

<style scoped>
.cross-border-payment {
  min-height: 100vh;
  position: relative;
  overflow: hidden;
  background: linear-gradient(135deg, var(--bg-gradient-start) 0%, var(--bg-gradient-mid) 50%, var(--bg-gradient-end) 100%);
  color: var(--text-primary);
  font-family: var(--font-body);
  padding: 2rem;
}

.grid-bg {
  position: fixed;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  background-image:
    linear-gradient(rgba(0, 255, 204, 0.03) 1px, transparent 1px),
    linear-gradient(90deg, rgba(0, 255, 204, 0.03) 1px, transparent 1px);
  background-size: 50px 50px;
  animation: cbpGridMove 20s linear infinite;
  pointer-events: none;
  z-index: 0;
}

.grid-bg-2 {
  background-image:
    linear-gradient(rgba(0, 255, 204, 0.02) 1px, transparent 1px),
    linear-gradient(90deg, rgba(0, 255, 204, 0.02) 1px, transparent 1px);
  background-size: 100px 100px;
  animation: cbpGridMove 30s linear infinite reverse;
}

@keyframes cbpGridMove {
  0% { transform: perspective(500px) rotateX(60deg) translateY(0); }
  100% { transform: perspective(500px) rotateX(60deg) translateY(50px); }
}

.cbp__skeleton {
  position: relative;
  z-index: 1;
  max-width: 900px;
  margin: 4rem auto;
  padding: 2rem;
}

.cbp__skeleton-block {
  background: var(--surface-elevated);
  border: 1px solid rgba(0, 255, 204, 0.2);
  border-radius: var(--radius-md);
  margin-bottom: 1rem;
  animation: cbpShimmer 1.5s ease-in-out infinite;
}

.cbp__skeleton-block--lg { height: 60px; }
.cbp__skeleton-block--md { height: 40px; width: 80%; }
.cbp__skeleton-block--sm { height: 30px; width: 50%; }

@keyframes cbpShimmer {
  0%, 100% { opacity: 0.6; }
  50% { opacity: 1; }
}

.cbp__content {
  position: relative;
  z-index: 1;
}

.cbp__breadcrumb {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  margin-bottom: 2rem;
  font-family: var(--font-body);
  font-size: 0.9rem;
  flex-wrap: wrap;
}

.cbp__breadcrumb-link {
  color: var(--text-secondary);
  text-decoration: none;
  transition: color 0.3s ease;
}

.cbp__breadcrumb-link:hover,
.cbp__breadcrumb-link:focus {
  color: var(--cyan);
}

.cbp__breadcrumb-separator {
  color: rgba(0, 255, 204, 0.5);
}

.cbp__breadcrumb-current {
  color: var(--cyan);
  font-weight: 500;
}

.cbp__hero {
  text-align: center;
  padding: 4rem 2rem;
  background: var(--surface-elevated);
  border: 1px solid rgba(0, 255, 204, 0.2);
  border-radius: var(--radius-lg);
  margin-bottom: 3rem;
  backdrop-filter: blur(10px);
}

.cbp__hero-icon-wrapper {
  margin-bottom: 1.5rem;
}

.cbp__hero-icon {
  font-size: 5rem;
  width: 120px;
  height: 120px;
  line-height: 120px;
  margin: 0 auto;
  border-radius: 50%;
  background: rgba(0, 255, 204, 0.1);
  display: flex;
  align-items: center;
  justify-content: center;
  animation: cbpFloat 3s ease-in-out infinite;
}

@keyframes cbpFloat {
  0%, 100% { transform: translateY(0); }
  50% { transform: translateY(-10px); }
}

.cbp__hero-title {
  font-family: var(--font-display);
  font-size: 2.5rem;
  font-weight: 700;
  margin: 0 0 1rem 0;
  line-height: 1.3;
  letter-spacing: 0.05em;
  color: var(--cyan);
}

.cbp__hero-subtitle {
  font-size: 1.2rem;
  color: var(--text-secondary);
  max-width: 800px;
  margin: 0 auto 2rem;
  line-height: 1.6;
}

.cbp__hero-cta {
  margin-top: 2rem;
}

.cbp__hero-button {
  display: inline-block;
  padding: 1rem 2.5rem;
  background: rgba(0, 255, 204, 0.1);
  border: 2px solid rgba(0, 255, 204, 0.4);
  border-radius: var(--radius-md);
  color: var(--cyan);
  text-decoration: none;
  font-family: var(--font-body);
  font-weight: 600;
  text-transform: uppercase;
  letter-spacing: 0.15em;
  transition: all 0.3s ease;
}

.cbp__hero-button:hover,
.cbp__hero-button:focus {
  background: rgba(0, 255, 204, 0.2);
  border-color: rgba(0, 255, 204, 0.6);
  box-shadow: 0 0 20px rgba(0, 255, 204, 0.3);
  transform: translateY(-2px);
}

.cbp__section-title {
  font-family: var(--font-display);
  font-size: 2rem;
  font-weight: 600;
  color: var(--text-primary);
  margin: 0 0 2rem 0;
  text-align: center;
  letter-spacing: 0.05em;
  position: relative;
}

.cbp__section-title::after {
  content: '';
  display: block;
  width: 100px;
  height: 2px;
  background: linear-gradient(90deg, transparent, var(--cyan), transparent);
  margin: 1rem auto 0;
}

.cbp__overview {
  max-width: 900px;
  margin: 0 auto 4rem;
  padding: 2rem;
  background: var(--surface-elevated);
  border: 1px solid rgba(0, 255, 204, 0.2);
  border-radius: var(--radius-lg);
}

.cbp__overview-text {
  font-size: 1.1rem;
  line-height: 1.8;
  color: var(--text-secondary);
  text-align: center;
}

.cbp__capabilities,
.cbp__core-features,
.cbp__tech-features {
  margin-bottom: 4rem;
}

.cbp__cards-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));
  gap: 2rem;
}

.cbp__card {
  background: var(--surface-elevated);
  border: 1px solid rgba(0, 255, 204, 0.2);
  border-radius: var(--radius-lg);
  padding: 2rem;
  text-align: center;
  transition: all 0.3s ease;
  animation: cbpFadeInUp 0.6s ease forwards;
  opacity: 0;
}

.cbp__card:hover {
  border-color: rgba(0, 255, 204, 0.5);
  transform: translateY(-5px);
  box-shadow: 0 8px 20px rgba(0, 255, 204, 0.15);
}

@keyframes cbpFadeInUp {
  from { opacity: 0; transform: translateY(20px); }
  to { opacity: 1; transform: translateY(0); }
}

.cbp__card-icon {
  font-size: 3rem;
  width: 80px;
  height: 80px;
  line-height: 80px;
  margin: 0 auto 1rem;
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
}

.cbp__card-title {
  font-family: var(--font-display);
  font-size: 1.2rem;
  font-weight: 600;
  color: var(--cyan);
  margin: 0 0 0.75rem 0;
}

.cbp__card-description {
  font-size: 0.95rem;
  line-height: 1.6;
  color: var(--text-secondary);
}

.cbp__cta {
  text-align: center;
  padding: 4rem 2rem;
  background: var(--surface-elevated);
  border: 2px solid rgba(0, 255, 204, 0.3);
  border-radius: var(--radius-lg);
  margin-bottom: 3rem;
}

.cbp__cta-title {
  font-family: var(--font-display);
  font-size: 2rem;
  font-weight: 700;
  color: var(--cyan);
  margin: 0 0 1rem 0;
}

.cbp__cta-description {
  font-size: 1.1rem;
  color: var(--text-secondary);
  margin-bottom: 2rem;
  max-width: 600px;
  margin-left: auto;
  margin-right: auto;
}

.cbp__cta-button {
  display: inline-block;
  padding: 1rem 2.5rem;
  background: rgba(0, 255, 204, 0.1);
  border: 2px solid rgba(0, 255, 204, 0.4);
  border-radius: var(--radius-md);
  color: var(--cyan);
  text-decoration: none;
  font-family: var(--font-body);
  font-weight: 600;
  text-transform: uppercase;
  letter-spacing: 0.15em;
  transition: all 0.3s ease;
}

.cbp__cta-button:hover,
.cbp__cta-button:focus {
  background: rgba(0, 255, 204, 0.2);
  border-color: rgba(0, 255, 204, 0.6);
  box-shadow: 0 0 20px rgba(0, 255, 204, 0.3);
  transform: translateY(-2px);
}

.cbp__back {
  text-align: center;
  margin-bottom: 3rem;
}

.cbp__back-link {
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
  color: var(--cyan);
  text-decoration: none;
  font-family: var(--font-body);
  font-size: 0.95rem;
  font-weight: 500;
  text-transform: uppercase;
  letter-spacing: 0.1em;
  transition: all 0.3s ease;
}

.cbp__back-link:hover,
.cbp__back-link:focus {
  color: var(--text-primary);
}

.cbp__back-icon {
  font-size: 1.2rem;
  transition: transform 0.3s ease;
}

.cbp__back-link:hover .cbp__back-icon {
  transform: translateX(-4px);
}

.neon-border {
  box-shadow:
    0 0 5px rgba(0, 255, 204, 0.5),
    0 0 10px rgba(0, 255, 204, 0.3),
    inset 0 0 5px rgba(0, 255, 204, 0.1);
}

.neon-text {
  text-shadow:
    0 0 5px rgba(0, 255, 204, 0.5),
    0 0 10px rgba(0, 255, 204, 0.3),
    0 0 15px rgba(0, 255, 204, 0.2);
}

@media (max-width: 768px) {
  .cross-border-payment {
    padding: 1.5rem 1rem;
  }

  .cbp__hero {
    padding: 3rem 1.5rem;
  }

  .cbp__hero-title {
    font-size: 1.75rem;
  }

  .cbp__cards-grid {
    grid-template-columns: 1fr;
  }

  .cbp__cta {
    padding: 3rem 1.5rem;
  }

  .cbp__cta-title {
    font-size: 1.5rem;
  }
}
</style>
