/**
 * @file ServiceDigitalAssetCustody.test.ts
 * @description Unit tests for the Digital Asset Custody Service detail (stub) view
...
import ServiceDigitalAssetCustody from '../ServiceDigitalAssetCustody.vue'
...
describe('ServiceDigitalAssetCustody.vue', () => {
  let wrapper: VueWrapper

  beforeEach(() => {
    wrapper = mount(ServiceDigitalAssetCustody, {
      global: {
        stubs: {
  ...
  describe('Rendering', () => {
    ...
    it('renders the hero section', () => {
      expect(wrapper.find('.dac__hero').exists()).toBe(true)
    })

    it('renders the page title as an h1', () => {
      const h1 = wrapper.find('.dac__hero h1')
      expect(h1.exists()).toBe(true)
      expect(h1.element.tagName.toLowerCase()).toBe('h1')
    })

    it('renders the overview section', () => {
      expect(wrapper.find('.dac__overview').exists()).toBe(true)
    })
    ...
    it('renders a breadcrumb link back to home', () => {
      const home = wrapper.find('.dac__breadcrumb a[href="/"]')
  ...
  describe('Content', () => {
    it('renders the hero heading via t()', () => {
      expect(wrapper.text()).toContain(
        (wrapper.vm as any).t('services.digitalAssetCustody.hero.heading'),
      )
    })
    ...
    it('renders the overview heading and description via t()', () => {
      ...
      expect(text).toContain(vm.t('services.digitalAssetCustody.overview.heading'))
      expect(text).toContain(vm.t('services.digitalAssetCustody.overview.description'))
    })
    ...
    it('renders the CTA button label via t()', () => {
      expect(wrapper.text()).toContain(
        (wrapper.vm as any).t('services.digitalAssetCustody.cta.button'),
      )
    })

    it('does NOT render a metrics section (stub view)', () => {
      expect(wrapper.find('.dac__metrics').exists()).toBe(false)
    })

    it('does NOT render a process section (stub view)', () => {
      expect(wrapper.find('.dac__process').exists()).toBe(false)
    })
  })
  ...
  describe('Internationalization', () => {
    it('does not leak raw services.digitalAssetCustody.* keys into rendered text', () => {
      const text = wrapper.text()
      const rawKeyPattern = /services\.digitalAssetCustody\.[a-zA-Z]+\.[a-zA-Z]+/g
    ...
    it('returns the key as fallback for a genuinely-missing key', () => {
      const result = (wrapper.vm as any).t('services.digitalAssetCustody.this.key.does.not.exist')
      expect(result).toBe('services.digitalAssetCustody.this.key.does.not.exist')
    })
  })

  describe('Edge Cases', () => {
    it('renders a loading skeleton initially and content after mount', async () => {
      const fresh = mount(ServiceDigitalAssetCustody)
      expect(fresh.find('.dac__skeleton').exists()).toBe(true)
      expect(fresh.find('.dac__content').exists()).toBe(false)

      await fresh.vm.$nextTick()
      await Promise.resolve()
      await fresh.vm.$nextTick()

      expect(fresh.find('.dac__skeleton').exists()).toBe(false)
      expect(fresh.find('.dac__content').exists()).toBe(true)
      fresh.unmount()
    })

    it('renders consistently across multiple mounts', () => {
      const w1 = mount(ServiceDigitalAssetCustody, {
        global: { stubs: { 'router-link': RouterLinkStub } },
      })
      const w2 = mount(ServiceDigitalAssetCustody, {
        global: { stubs: { 'router-link': RouterLinkStub } },
      })
